...
class BaseTrainer:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        with torch_distributed_zero_first(LOCAL_RANK):  # avoid auto-downloading dataset multiple times
            self.trainset, self.testset, self.queryset = self.get_dataset()
        self.ema = None
    ...
    def _setup_scheduler(self):
        ...
        self.scheduler_query = optim.lr_scheduler.LambdaLR(self.optimizer_query, lr_lambda=self.lf)
    ...
    def _setup_train(self, world_size):
        ...
        self.query_loader = self.get_dataloader(self.queryset, batch_size=batch_size, rank=LOCAL_RANK, mode="train")
        if RANK in {-1, 0}:
        ...
        self.optimizer_query = self.build_optimizer(
            model=self.model,
            name=self.args.optimizer,
            lr=self.args.lr0,
            momentum=self.args.momentum,
            ...
            for i, batch in pbar:
                ...
                for i, batch in pbar_query:
                    ...
                    if ni <= nw:
                        xi = [0, nw]  # x interp
                        self.accumulate = max(1, int(np.interp(ni, xi, [1, self.args.nbs / self.batch_size]).round()))
                        for j, x in enumerate(self.optimizer_query.param_groups):
                            # Bias lr falls from 0.1 to lr0, all other lrs rise from 0.0 to lr0
                            x["lr"] = np.interp(
                                ni, xi, [self.args.warmup_bias_lr if j == 0 else 0.0, x["initial_lr"] * self.lf(epoch)]
                            )
    ...
    def get_dataset(self):
        ...
        self.data = data
        return data["train"], data.get("val") or data.get("test"), data['query']
    ...
    def optimizer_query_step(self):
        """Perform a single step of the training optimizer with gradient clipping and EMA update."""
        self.scaler.unscale_(self.optimizer_query)  # unscale gradients
        torch.nn.utils.clip_grad_norm_(self.model.parameters(), max_norm=10.0)  # clip gradients
        self.scaler.step(self.optimizer_query)
        self.scaler.update()
